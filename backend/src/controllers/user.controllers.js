...
exports.avatarUpdate = async (req, res) => {
  try {
    ...
    if (file) {
      ...
      const updatedUser = await prisma.users.update({
        where: { id: user.id },
        data: { avatar: `/uploads/users/${file.filename}` }
      });
...
exports.getUsersList = async (req, res) => {
  try {
    ...
    // finding all users data from database (for total count)
    ...
    const queryHelper = new MyQueryHelper(req.query).search('fullName').sort().paginate();
    const queryOptions = queryHelper.getQueryOptions();
    const findUsers = await prisma.users.findMany(queryOptions);
    ...
    res.status(200).json(successResponse(
      ...
      'Users list data found successful',
      {
        ...
        total_page: req?.query?.keyword ? Math.ceil(findUsers.length / req.query.limit) : Math.ceil(totalUsers / req.query.limit),
...
exports.blockedUser = async (req, res) => {
  try {
    ...
    const blockedUser = await prisma.users.update({
      where: { id: user.id },
      data: { status: 'blocked' }
    });
...
exports.unblockedUser = async (req, res) => {
  try {
    ...
    // update user status & updateAt time
    const unblockedUser = await prisma.users.update({
      where: { id: user.id },
      data: { status: 'logout' }
    });
